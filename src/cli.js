...
export class CLI {
     ...
     * @type {Record<string,string>|ProcessEnv}
     */
    #env;
    ...
    async execute(args) {
        ...
        if (!flags.org || !flags.repo || !flags.name) {
            this.#console.error("Missing required arguments: --org, --repo, --name");
            this.showHelp();
            return 1;
        }
        
        const { org, repo, name } = flags;
        ...
        try {
            ...
            this.#console.error(`Error: ${/** @type {Error} */ (error).message}`);
            return 1;
        }
     ...
     */
    showHelp() {
        this.#console.log("Usage: node cli.js --org <org> --repo <repo> --name <name>");
